...
interface LayoutState {
   ...
   *
   * Existing user-typed names are preserved by grid position (rowIndex,
   * columnIndex); only genuinely new cells get default names.
   *
...
function regenCells(
  ...
  return generateGridHomography(ordered, s.layout.rowCount, s.layout.columnsPerRow, {
    canonicalize: false,
...
export const useLayoutStore = create<LayoutState>()(
  temporal(
    (set) => ({
      ...
      setGridOffset: (axis, v) =>
        set((s) => {
          ...
          const fresh = regenCells(
            ...
            axis === 'x' ? { offsetX: clamped } : { offsetY: clamped },
          );
          const cells = mergeNamesByRowCol(fresh, s.layout.cells);
          return {
            ...patch,
      ...
      setStationName: (stationName) => set((s) => ({ layout: { ...s.layout, stationName } })),
      ...
      applyHomographyFix: (opts) =>
        set((s) => {
          ...
          const fresh = regenCells({ ...s, layout: { ...s.layout, boundaryCorners: ordered } });
          // Preserve any names the user already typed (matched by grid position)
          // so Auto Fix corrects geometry WITHOUT wiping labels.
          const cells = mergeNamesByRowCol(fresh, s.layout.cells);
          return {
            layout: {
              ...s.layout,
